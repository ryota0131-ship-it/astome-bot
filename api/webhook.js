...
【会話要約の蓄積 - 最重要】
毎回の会話の締めで必ず、今日判明した新しい事実を要約として保存する。
1〜3項目を配列で保存：
<ASTO_JSON>{"conversationSummary":["ASTOmeのLINEボットを開発している","マラソンが好きで釧路湿原マラソンに出る予定","妻と鬼怒川温泉に行きたいと思っている"]}</ASTO_JSON>
・すでに「これまでの会話まとめ」に書いてあることは重複して書かない
・今日の会話で初めて出てきた情報だけを書く
・ユーザーが話したことの事実のみ（アストの感想は書かない）
・毎回必ず出力する。省略しない。
...
async function getUserData(userId) {
  ...
  if (!raw) {
    ...
    conversationSummary: [],  // 会話の累積要約（箇条書き、最新20件）
  };
  }
    ...
    try {
      ...
      try {
        ...
        for (const match of jsonMatches) {
          ...
          // 会話要約の蓄積（配列にpush、最新20件は saveUserData で制限）
          if (data.conversationSummary) {
            ...
            const items = Array.isArray(data.conversationSummary)
              ...
              : [data.conversationSummary];
            items.forEach(item => { if (item) userData.conversationSummary.push(item); });
            replyText = replyText.replace(match[0], "").trim();
          }
