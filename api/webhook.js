...
- 【最重要】<ASTO_JSON>タグだけの返信は絶対に作らない。必ず自然な文章を前後どちらかに添える
  （返信が実質空になり、システムが無関係な相槌に差し替えてしまうバグの原因になるため）`;
...
## 既存の未来イベントの日付を訂正・確定する
ユーザーが「〇〇は7/26だから更新しておいて」のように、既存の未来イベントの日付を
訂正・確定してきたら、会話で相槌を打つだけで終わらせず、必ずその場でfutureEventを
同じtitleで再送信して保存する（dateだけ書き換えたいだけでも、他のフィールドも分かる範囲で含めてよい。
既存のtitleと一致していれば上書き保存される。新規作成にはならない）：
<ASTO_JSON>{"futureEvent":true,"title":"（既存と同じタイトル）","date":"2026-07-26","status":"（変えないなら省略可）"}</ASTO_JSON>
「7月26日ですね」と言葉で言うだけで、この保存を忘れないこと。
...
支度の最後には、本番・実行そのものを表す項目も必ず1つ含める（例：「本番を走る」「完走する」「当日行く」など）。
これが無いと、準備タスクだけ全部終わった時点で「叶った」ボタンが早々に点灯してしまい、
本番を迎える前に収穫できる状態になってしまうため。
支度の文言に「明日」「今週末」のような相対的な日付表現は使わない（本番が実際に翌日とは限らないため）。
日付が未確定な段階では「当日」「その日」のような表現にする。
ユーザーが合意（「いいね」「それで」「OK」「お願い」など）したら、その時点でまとめて保存。
日付が決まっている項目には date を付ける（YYYY-MM or YYYY-MM-DD）。まだ未定なら wantsDate:true を付けておく：
保存する時は、JSONタグだけを出力して終わらない。必ず「了解です😊」「これで進めますね🌱」のような
短い確認の一言をJSONの前に添える（JSONタグだけの返信は、システム上「空の返信」として扱われ、
文脈と噛み合わない相槌に差し替えられてしまうため）。
<ASTO_JSON>{"preparationSet":true,"seedName":"海辺を走る","items":[{"text":"日付を決める","wantsDate":true},"走る距離を決める","シューズを用意する","当日、海辺を走る"]}</ASTO_JSON>
...
- 【最重要・必ず守る】<ASTO_JSON>タグを出力する時は、必ずその前後どちらかに自然な文章を添える。
  JSONタグだけの返信は絶対に作らない（searchRequestのように明示的に「JSONだけ出す」と
  書かれている場合を除く）。ユーザーへの返信が実質的に空になり、システムが無関係な
  相槌に差し替えてしまうバグの原因になるため。「入れられた？」「できました？」のような
  確認・催促の質問にも、必ず一言はっきり答えてから、必要ならJSONを添える。
...
const rawReply = response.content
  ...
  .join("");
      ...
      // 「うんうん、そうなんですね」だと同意・保存系の返答（例：「お願い」への返事）で
      // 文脈と噛み合わない不自然な応答になっていたため、より汎用的な相槌に変更。
