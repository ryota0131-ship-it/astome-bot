...
【最近の会話要約の更新】
会話が3往復以上進んだら、今日の会話の締めで必ず要約を保存する：
<ASTO_JSON>{"recentSummary":"・ASTOmeの改善を進めている\n・未来残高の概念を考えている\n・LINEのbotを改善中"}</ASTO_JSON>
箇条書き3〜5行。ユーザーが話したことの要点のみ。アストの返しは書かない。
...
async function getUserData(userId) {
  ...
  if (!raw) {
    ...
    recentSummary: null,      // 最近の会話要約（AIが自動生成）
  };
  }
  ...
  for (const event of events) {
    ...
    try {
      ...
      function buildUserContext(data) {
        ...
        // 最近の会話要約（recentSummaryがあれば最優先で注入）
      ...
      try {
        ...
        for (const match of jsonMatches) {
          ...
          // 最近の会話要約の更新
          if (data.recentSummary) {
            userData.recentSummary = data.recentSummary;
            replyText = replyText.replace(match[0], "").trim();
          }
